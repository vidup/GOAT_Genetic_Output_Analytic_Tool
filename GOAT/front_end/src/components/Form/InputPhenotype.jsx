...
//Component
var InputPhenotype = React.createClass({
    ...
    this.setState({value : e.target.value},
      function(){
        if(this.state.value.length>=2){
          ...
          xhr.open('GET', encodeURI("/autocomplete/phenotypes/"+this.state.value), true);
          var component = this;
          xhr.onload = function(){
            if(xhr.status==200){
  ...
  render : function(){
    ...
    var phenotypeComponent = function(phenotype, index){
      return <option key={index+phenotype.nom}>{phenotype.nom}</option>
    }
    ...
    return (
      <div>
        <input
          ...
          list="listPhenotypes"
        />
        <datalist id="listPhenotypes">{this.state.autoCompleteData.map(phenotypeComponent)}</datalist>
      </div>
    );
  }
});

module.exports= InputPhenotype;
